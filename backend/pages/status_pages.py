...
class LibStatusPage(ctk.CTkFrame):
    ...
    def create_ui(self):
        ...
        self.deno_info_label.pack(anchor="w", padx=10, pady=(2, 2))
        ctk.CTkLabel(deno_card, text="(Bypasses YT signature decryption)", font=ctk.CTkFont(size=7), text_color="gray", wraplength=150, justify="left").pack(anchor="w", padx=10, pady=(0, 5))
        ...
        # ===== Optional Python Packages Section =====
        python_section = ctk.CTkFrame(main, fg_color=("gray90", "gray17"))
        python_section.pack(fill="x", pady=(10, 10))
        
        ctk.CTkLabel(python_section, text="Optional Local AI Packages (For Free Offline Processing)", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=15, pady=(12, 8))
        
        # Grid - 2 columns
        py_cards_frame = ctk.CTkFrame(python_section, fg_color="transparent")
        py_cards_frame.pack(fill="x", padx=10, pady=(0, 12))
        py_cards_frame.grid_columnconfigure((0, 1), weight=1, uniform="py")
        
        # faster-whisper card
        whisper_card = ctk.CTkFrame(py_cards_frame, fg_color=("gray85", "gray20"), corner_radius=10, border_width=1, border_color=("#cbd5e1", "#2d2e36"))
        whisper_card.grid(row=0, column=0, padx=5, pady=5, sticky="nsew")
        ctk.CTkLabel(whisper_card, text="🎙️ faster-whisper", font=ctk.CTkFont(size=11, weight="bold")).pack(anchor="w", padx=10, pady=(10, 5))
        self.whisper_status_label = ctk.CTkLabel(whisper_card, text="Checking...", font=ctk.CTkFont(size=10), text_color="gray")
        self.whisper_status_label.pack(anchor="w", padx=10)
        self.whisper_info_label = ctk.CTkLabel(whisper_card, text="Local offline transcription", font=ctk.CTkFont(size=9), text_color="gray")
        self.whisper_info_label.pack(anchor="w", padx=10, pady=(2, 5))
        self.whisper_install_btn = ctk.CTkButton(whisper_card, text="📥 Install", height=26, font=ctk.CTkFont(size=10),
            command=lambda: self.install_python_package("faster-whisper", self.whisper_status_label, self.whisper_install_btn), fg_color=("#3B8ED0", "#1F6AA5"))
        
        # gTTS card
        gtts_card = ctk.CTkFrame(py_cards_frame, fg_color=("gray85", "gray20"), corner_radius=10, border_width=1, border_color=("#cbd5e1", "#2d2e36"))
        gtts_card.grid(row=0, column=1, padx=5, pady=5, sticky="nsew")
        ctk.CTkLabel(gtts_card, text="🔊 gTTS (Google TTS)", font=ctk.CTkFont(size=11, weight="bold")).pack(anchor="w", padx=10, pady=(10, 5))
        self.gtts_status_label = ctk.CTkLabel(gtts_card, text="Checking...", font=ctk.CTkFont(size=10), text_color="gray")
        self.gtts_status_label.pack(anchor="w", padx=10)
        self.gtts_info_label = ctk.CTkLabel(gtts_card, text="Local offline hook voices", font=ctk.CTkFont(size=9), text_color="gray")
        self.gtts_info_label.pack(anchor="w", padx=10, pady=(2, 5))
        self.gtts_install_btn = ctk.CTkButton(gtts_card, text="📥 Install", height=26, font=ctk.CTkFont(size=10),
            command=lambda: self.install_python_package("gtts", self.gtts_status_label, self.gtts_install_btn), fg_color=("#3B8ED0", "#1F6AA5"))
    ...
    def refresh_status(self):
        ...
        self.whisper_status_label.configure(text="Checking...", text_color="gray")
        self.gtts_status_label.configure(text="Checking...", text_color="gray")
        
        # Hide download buttons
        self.ffmpeg_download_btn.pack_forget()
        self.ffmpeg_reinstall_btn.pack_forget()
        self.deno_download_btn.pack_forget()
        self.whisper_install_btn.pack_forget()
        self.gtts_install_btn.pack_forget()
        ...
        def check_libs():
            ...
            # Check faster-whisper status
            try:
                import faster_whisper
                self.after(0, lambda: self.whisper_status_label.configure(text="✓ Installed", text_color="green"))
            except ImportError:
                self.after(0, lambda: self.whisper_status_label.configure(text="✗ Not Installed", text_color="orange"))
                self.after(0, lambda: self.whisper_install_btn.pack(padx=10, pady=(2, 10), fill="x"))
                
            # Check gTTS status
            try:
                import gtts
                self.after(0, lambda: self.gtts_status_label.configure(text="✓ Installed", text_color="green"))
            except ImportError:
                self.after(0, lambda: self.gtts_status_label.configure(text="✗ Not Installed", text_color="orange"))
                self.after(0, lambda: self.gtts_install_btn.pack(padx=10, pady=(2, 10), fill="x"))
    ...
    def install_python_package(self, package_name: str, status_label: ctk.CTkLabel, install_btn: ctk.CTkButton):
        """Run pip install for a package in the background"""
        status_label.configure(text="Installing...", text_color="orange")
        install_btn.configure(state="disabled")
        
        def run_install():
            import subprocess
            try:
                # Use current Python executable (supports venv automatically)
                ...
                if result.returncode == 0:
                    self.after(0, lambda: status_label.configure(text="✓ Installed", text_color="green"))
                    self.after(0, lambda: install_btn.pack_forget())
                else:
                    self.after(0, lambda: status_label.configure(text="✗ Failed", text_color="red"))
                    self.after(0, lambda: install_btn.configure(state="normal"))
